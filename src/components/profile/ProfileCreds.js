  ...
  render(){
    ...
    const expItems = experience.map(exp => (
      <li key={ exp._id } className="list-group-item">
        <h4>{exp.company}</h4>
        <p>
          <Moment format="YY/MM">{ exp.from }</Moment>{' to '}
          { exp.to === null ? ('Now') : (<Moment format="YY MM">{ exp.to }</Moment>) }
        </p>
    ...
    const eduItems = education.map(edu => (
      <li key={ edu._id } className="list-group-item">
        <h4>{edu.school}</h4>
        <p>
          <Moment format="YY/MM">{ edu.from }</Moment>{' to '}
          { edu.to === null ? ('Now') : (<Moment format="YY/MM">{ edu.to }</Moment>) }
        </p>
